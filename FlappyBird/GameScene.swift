...
class GameScene: SKScene, SKPhysicsContactDelegate{
    ...
    let playSound = SKAction.playSoundFileNamed("coin04.mp3", waitForCompletion: false)
    ...
    func setupCoin() {
        ...
        let createCoinAnimation = SKAction.run({
            ...
            let random_y_range = self.frame.size.height / 4
            // アイテムのY軸の下限
            let item_lowest_y = UInt32( center_y - coinTexture.size().height / 2 -  random_y_range / 2)
            // 1〜random_y_rangeまでのランダムな整数を生成
            let random_y = arc4random_uniform( UInt32(random_y_range) )
            ...
            //コインを作成
            let doller = SKSpriteNode(texture: coinTexture)
            doller.position = CGPoint(x: self.frame.size.width + coinTexture.size().width / 2 + doller.frame.width, y: item_y)
            
            // 物理演算を設定
            doller.physicsBody = SKPhysicsBody(circleOfRadius: doller.frame.width)
            doller.physicsBody?.affectedByGravity = false
            
            // 衝突のカテゴリー設定
            doller.physicsBody?.categoryBitMask = self.itemCategory
            
            // 衝突の時に動かないように設定する
            doller.physicsBody?.isDynamic = false
            
            //アニメーションを設定
            doller.run(coinAnimation)
            
            // スプライトを追加する
            self.coinNode.addChild(doller)
